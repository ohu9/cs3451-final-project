...
struct AreaLight 
{
    vec3 corner;                    /* position of bottom left corner of area light */
    vec3 uvec;                      /* vector defining u side of light */
    vec3 vvec;                      /* vector defining v side of light */
    int usteps;                     /* int defining numer of steps on the u edge */
    int vsteps;                     /* int defining numer of steps on the v edge */
    int numCells;                   /* usteps * vsteps */
    vec3 Ia;                        /* ambient intensity */
    vec3 Id;                        /* diffuse intensity */
    vec3 Is;                        /* specular intensity */     
};
...
void initScene() 
{
    ...
    areaLights[0] = AreaLight(vec3(-1, 10, 1), 
                            /*uvec*/vec3(4,0,0),
                            /*vvec*/vec3(0,4,0),
                            /*usteps*/ 8,
                            /*vsteps*/ 8,
    ...
    spheres[0] = Sphere(vec3(0, .8, -1), .8, 1);
    spheres[1] = Sphere(vec3(1.1, 0.4, -0.8), 0.4, 1);
    spheres[2] = Sphere(vec3(-1.4, 0.5, -0.8), 0.5, 1);
}
...
vec3 sampleDiffuse(int matId, vec3 p) 
{
    ...
    if(matId == 0 || matId == 1) {
		vec2 uv = vec2(p.x, p.z*.5) / 5.0;     /* uv texture on the ground */
...
bool isShadowed(vec3 light, Hit h) 
{
    bool shadowed = false;                          /* returned boolean to indicate if there is shadow or not */
	vec3 intersect = h.p;                           /* hit intersect */
	vec3 toLight = light-intersect;        /* vector pointing from the shadow ray origin to the light */
...
vec3 pointOnLight(AreaLight light, int i, int j) {
    float jitter = clamp(base_hash(uvec2(i,j)), -1., 1.);
    return light.corner +
         light.uvec * (i + 0.5 + jitter) +
         light.vvec * (j + 0.5 + jitter);
}

float intensityAt(AreaLight light, vec2 uv, Hit h) {
    float total = 0.;

    for (int i = 0; i < light.vsteps; i++) {
        for (int j = 0; j < light.usteps; j++) {
            vec3 light_position = pointOnLight(light, i, j);
            if (!isShadowed(light_position, h)) {
                total += 1.;
            }
        }
    }
    return total / (light.numCells);
}
...
vec3 rayTrace(in Ray r, out Hit hit, vec2 uv) 
{
    vec3 col = vec3(0);
    Hit h = findHit(r);
    hit = h;
...
void main() 
{
    ...
    for(int i = 0; i < recursiveDepth; i++) {
        Hit hit;
        vec3 col = rayTrace(recursiveRay, hit, uv);
